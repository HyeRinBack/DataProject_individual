...
### 사용자리뷰 데이터의 체형을 통해 몇가지 가설 추가 선정
**3. 가설 6. 체형에 따라 선호하는 특정 카테고리가 존재할 것이다.**
- 체형별 각 카테고리(대분류)에 대한 리뷰수, 평균 사용자별점 비교
- 소분류에 대한 선호도 분석

**4. 가설 7. 특정 체형을 가진 고객의 브랜드 충성도가 높을 것이다.**
- 각 체형별 매출량 분석을 통해 재구매 의향 및 충성고객 분석
...
#### 4.2.6 가설6 검증 진행
- 귀무가설: 체형별 카테고리 리뷰수와 평균 별점은 선호도와 유의미한 상관관계가 없을 것이다.
