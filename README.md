...
**1. 특정 카테고리에 따라 감성 점수가 높을 것이다.**
- 대분류, 소분류 따로 감성 점수 평균을 구해 해당 분포를 시각화
예상 인사이트 - 의류 카테고리에 따라 긍정, 부정 비율이 다를 것(평균이 낮으면 - 불만이 많음 / 평균이 높으면 - 만족도가 높음)

**2. 특정 브랜드는 타 브랜드에 비해 감성 점수가 높을 것이다.**
- 브랜드별 감성점수 평균을 비교하고, 긍/부정 리뷰 비율을 비교
예상 인사이트 - 브랜드별 고객 충성도 차이 분석 가능

**3. 특정 체형에 따라 감성점수 차이가 존재할 것이다.**
- 성별에 따른 감성점수 평균 / 체형에 따른 감성점수 관계를 비교하여 시각화

**4. 특정 키워드가 포함된 리뷰에서 감성점수가 극단적일 것이다.(긍/부정 리뷰 요인 비교)**
- TF/IDF 기반 키워드를 추출하고 감성점수와 비교
  - 1, 2점 리뷰 단어 / 4, 5점 리뷰 단어를 워드 클라우드로 비교
예상 인사이트 - 긍/부정 주요 키워드를 통해 브랜드 상품 개선 포인트 발견
